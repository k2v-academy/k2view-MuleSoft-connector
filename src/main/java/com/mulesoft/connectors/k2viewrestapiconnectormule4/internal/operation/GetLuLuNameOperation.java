...
import com.mulesoft.connectors.k2viewrestapiconnectormule4.internal.metadata.GetLuLuNameOutputMetadataResolver;
...
public class GetLuLuNameOperation extends BaseRestOperation {
  private static final String OPERATION_PATH = "/lu/{luName}";

  private static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT =
      RestRequestBuilder.QueryParamFormat.MULTIMAP;

  public GetLuLuNameOperation() {}

  /**
   * Get lu by lu name
   *
   * <p>This operation makes an HTTP GET request to the /lu/{luName} endpoint
   *
   * @param config the configuration to use
   * @param connection the connection to use
   * @param luNameUriParam Lu Name
   * @param zipQueryParam download logical unit project files
  ...
  @DisplayName("Get lu by lu name")
  @MediaType("application/json")
  @OutputResolver(output = GetLuLuNameOutputMetadataResolver.class)
  public void getLuLuName(
      @Config RestConfiguration config,
      @Connection RestConnection connection,
      @DisplayName("Lu Name") String luNameUriParam,
      @Optional @DisplayName("zip") @Summary("download logical unit project files")
          boolean zipQueryParam,
    ...
    RestRequestBuilder builder =
        new RestRequestBuilder(
            ...
            .addQueryParam("zip", zipQueryParam);
    doRequest(
        config,
        connection,
        builder,
